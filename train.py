...
def preprocess(
    ...
    if use_incontext:
        incontext_input_ids, incontext_labels, incontext_attn = update_decoder_input_and_label(input_ids, sources_tokenized["input_ids_lens"], tokenizer)
        for label, source_len in zip(labels, sources_tokenized["input_ids_lens"]):
            label[:source_len] = IGNORE_INDEX
        return dict(input_ids=input_ids, labels=labels, incontext_input_ids=incontext_input_ids, incontext_labels=incontext_labels, incontext_attention_mask=incontext_attn)
    else:
        for label, source_len in zip(labels, sources_tokenized["input_ids_lens"]):
            label[:source_len] = IGNORE_INDEX
        ...
        if 'instruction' in list_data_dict[0]:
            ...
            def get_input(query):
                if query.find('\n') == -1:
                    return ''
                return '\n'.join(query.split('\n')[1:])
            list_data_dict = [{'instruction':data['question'].split('\n')[0], 'input':get_input(data['question']), 'output':data['explanation']} for data in list_data_dict]
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        data_dict = preprocess(sources, targets, self.tokenizer, self.use_incontext)
        # input_ids, labels = tuple([instance[key] for instance in instances] for key in ("input_ids", "labels"))
        if self.use_incontext:
            input_ids, labels, incontext_input_ids, incontext_labels, incontext_attention_mask = data_dict['input_ids'], data_dict['labels'], \
                data_dict['incontext_input_ids'], data_dict['incontext_labels'], data_dict['incontext_attention_mask']
            input_ids = torch.nn.utils.rnn.pad_sequence(
                input_ids, batch_first=True, padding_value=self.tokenizer.pad_token_id
            )
            labels = torch.nn.utils.rnn.pad_sequence(labels, batch_first=True, padding_value=IGNORE_INDEX)
            return dict(
                ...
                labels=labels,
                attention_mask=input_ids.ne(self.tokenizer.pad_token_id),
                incontext_input_ids=incontext_input_ids,
                incontext_labels=incontext_labels,
                incontext_attention_mask=incontext_attention_mask
            )
        else:
...
def make_supervised_data_module(tokenizer: transformers.PreTrainedTokenizer, data_args) -> Dict:
    ...
    data_collator = DataCollatorForSupervisedDataset(tokenizer=tokenizer, use_incontext=True)
...
def train():
    ...
    use_incontext = True
    if use_incontext:
        trainer = IncontextTrainer(alpha=0.8, model=model, tokenizer=tokenizer, args=training_args, **data_module)
    else:
...
if __name__ == "__main__":
    ...
    # parser.add_argument('--use_incontext', action='store_true', default=False)
